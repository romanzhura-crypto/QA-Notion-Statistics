...
def snapshot() -> dict:
    ...
    payload = {
        ...
        "status_dwell_note": "Notion REST does not expose Status changelog. Days = calendar time from Start date (fallback created_time) until now, or until last_edited_time if Status is Done.",
    ...
    try:
        ...
        # CI / non-QA hosts: workspace JSON is enough for publish.
        pass
    return payload
...
if __name__ == "__main__":
    ...
    if cmd == "serve":
        ...
        raise SystemExit("usage: release-widget-sync.py [snapshot|dry|serve]")
